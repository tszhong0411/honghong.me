...
export default function Footer() {
  ...
  const releaseHandler = (e) => {
    if (e.target.id === 'versionHistoryWrapper' || e.target.id === 'version') {
              ...
              <div key={index} className="w-full sm:w-1/2 md:w-1/4">
                ...
                <div className="flex flex-col">
                  {item.list.map((item, index) => {
                    return (
                      <Link key={index} href={item.href} className="my-1 mr-auto text-[16px]">
                        <i className={item.class}></i> {item.title}
                      </Link>
                    )
                  })}
                </div>
        ...
        <div className="mx-auto mb-8 flex w-full max-w-3xl flex-wrap justify-between px-[20px] xl:max-w-5xl">
          ...
          <div>
            {footerNavLinks.bottomLinks.map((item, index) => {
              return (
                <Link key={index} href={item.href} className="mr-4 text-base font-bold">
                  {item.title}
                </Link>
              )
            })}
